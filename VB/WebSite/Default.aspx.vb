...
Public Partial Class _Default
	...
	Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
		If Session(SessionDataSource1) Is Nothing Then
			Dim parameters1 As DataSourceParameters = New DataSourceParameters("~/App_Data/nwind.mdb", "SELECT [CategoryID], [CategoryName], [Description] FROM [Categories]")
			Session(SessionDataSource1) = parameters1
		End If
		If Session(SessionDataSource2) Is Nothing Then
			Dim parameters2 As DataSourceParameters = New DataSourceParameters("~/App_Data/nwind.mdb", "SELECT [ProductID], [ProductName], [CategoryID], [UnitPrice], [QuantityPerUnit] FROM [Products] WHERE [CategoryID] = @CategoryID")
	...
	Protected Function RestoreAccessDataSourceFormSessionParameters(ByVal dataSourceParametersSessionKey As String) As AccessDataSource
		...
		If parameters Is Nothing Then
            Return Nothing
		End If
		Dim dataSource As AccessDataSource = New AccessDataSource(parameters.DataFile, parameters.SelectComand)
